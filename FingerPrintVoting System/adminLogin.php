...
if(isset($_POST['submit'])){
 ...
 $db = mysqli_select_db($conn, "fingerprint");
  ...
  $query = "SELECT * FROM admin WHERE A_Pass='$pass' AND A_NID='$user'";
...
$result=mysqli_query($conn,$query);
 ...
 $_SESSION['loc']=$row["A_Location"];
 $loc=$row["A_Location"];
 $_SESSION['nid']=$row["A_NID"];
 $query1 = "SELECT * FROM election WHERE Area_Info like '%$loc%' order by E_Date ASC";
 //sql query to fetch information of registerd user and finds user match.
